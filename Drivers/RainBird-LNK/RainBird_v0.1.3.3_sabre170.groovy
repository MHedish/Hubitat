...
*  0.1.3.3s –– Custom revision for sabre170 testing.
*/
...
@Field static final String DRIVER_NAME     = "Rain Bird LNK/LNK2 WiFi Module Controller - saber170 edition"
...
private getAvailableStations(){
	try{
		...
		if(d.startsWith("83")){
			...
			if(isLegacyFirmware(2.11)&&hex.toUpperCase().startsWith("FF")){zones=(1..22).toList()
			...
			if(f?.startsWith("83")){def m=Integer.parseInt(f.substring(4,12),16);zones=(1..22).findAll{i->(m&(1<<(i-1)))!=0};emitChangedEvent("availableStations",zones.join(","),"Available stations: ${zones.join(', ')}")}
...
private verifyActiveZone(data=null,passive=false){
	...
	if(hybrid||getCommandSupport("3F")){
		...
		if(d?.startsWith("BF")){
			mask=Integer.parseInt(d.substring(4,6),16);def active=0;(1..22).each{i->if((mask&(1<<(i-1)))!=0)active=i}
...
private parseProgramZones(hex){
    ...
    def mask=Integer.parseInt(hex.substring(hex.length()-2),16)
    def zones=(1..22).collect{mask&(1<<(it-1))?it:null}.findAll{it}
    return zones
}
